...
class _ChatsScreenState extends State<ChatsScreen>
  ...
  // нужно для двухпанельного режима на десктопе
  ...
  void _openChat(ChatThread thread) {
    ...
    // на десктопе открываем во второй панели
    if (isDesktop(context)) {
      setState(() {
        _selectedThread = thread;
      });
    } else {
      // на мобилке уводим на отдельный экран
      Navigator.push(
        context,
        MaterialPageRoute(
  ...
  void _openUserChat(UserSearchItem user) async {
    ...
    if (threadId != null && mounted) {
      ...
      if (isDesktop(context)) {
        // перечитываем список, чтобы найти новый тред и выделить его
  ...
  Future<void> _searchMessages() async {
    ...
    if (mounted) _viewModel.loadThreads();
  }

  Widget _extraActions(AppLocalizations l10n) => PopupMenuButton<String>(
    tooltip: l10n.more,
    child: Container(
      width: 44,
      height: 44,
    ...
    itemBuilder: (fontContext) => [
      PopupMenuItem(value: 'group', child: Text(l10n.newGroup, style: appFont(fontContext))),
      PopupMenuItem(value: 'search', child: Text(l10n.searchMessages, style: appFont(fontContext))),
      PopupMenuItem(value: 'staff', child: Text(l10n.employeeDirectory, style: appFont(fontContext))),
    ],
  );
...
// заглушка правой панели на десктопе
