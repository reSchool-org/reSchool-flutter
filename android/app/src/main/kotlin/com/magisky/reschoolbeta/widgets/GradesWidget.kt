...
class GradesWidget : AppWidgetProvider() {
    ...
    companion object {
        fun updateWidget(
            ...
            val views = RemoteViews(context.packageName, R.layout.grades_widget)

            val intent = Intent(context, WidgetListService::class.java)
            intent.putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, appWidgetId)
            intent.putExtra("widget_type", "grades")
            ...
            val jsonData = prefs.getString("widget_grades_data", null)

            if (jsonData != null) {
                try {
                    val data = JSONObject(jsonData)
                    val periodName = data.optString("periodName", "")
                    if (periodName.isNotEmpty()) {
                        views.setTextViewText(R.id.widget_last_updated, periodName)
                    }

                    val grades = data.optJSONArray("grades")
