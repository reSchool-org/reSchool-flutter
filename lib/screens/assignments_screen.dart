...
class _AssignmentsScreenState extends State<AssignmentsScreen>
  ...
  DateTime? _selectedDate; // фильтр по дате, нужен только на десктопе
  String? _selectedSubject; // фильтр по предмету, тоже десктопный
      ...
      child: Listener(
        onPointerSignal: (event) {
          if (event is PointerScrollEvent &&
              _subjectScrollController.hasClients) {
            final maxExtent = _subjectScrollController.position.maxScrollExtent;
            if (maxExtent > 0) {
              final newOffset =
                  (_subjectScrollController.offset + event.scrollDelta.dy)
                      .clamp(0.0, maxExtent);
  ...
  Widget _buildMobileLayout(
    ...
    final filteredItems = _selectedSubject != null
        ...
        : vm.items;
    final filteredGrouped = _groupByDate(filteredItems);
    final filteredDates = filteredGrouped.keys.toList()
      ..sort((a, b) => b.compareTo(a));
    ...
    return Column(
      ...
      children: [
        ...
        // чипы предметов держим вне CustomScrollView, иначе не крутится колёсиком
        if (!vm.isLoading && vm.items.isNotEmpty)
          Padding(
            padding: const EdgeInsets.only(bottom: 8),
            child: _buildSubjectFilterChips(
              availableSubjects,
              colorScheme,
              l10n,
            ),
                    ...
                    delegate: SliverChildBuilderDelegate(
                      (context, index) {
                        ...
                        for (var date in filteredDates) {
                          final items = filteredGrouped[date]!;
                      ...
                      childCount: filteredDates.fold<int>(
                        0,
                        (sum, date) => sum + 1 + filteredGrouped[date]!.length,
                      ),
                    ),
                  ),
  ...
  Widget _buildDesktopLayout(
    ...
    // если предмет выбран, фильтруем по нему
    final filteredItems = _selectedSubject != null
        ? vm.items.where((item) => item.subject == _selectedSubject).toList()
        : vm.items;
    final filteredGroupedItems = _groupByDate(filteredItems);
    final filteredSortedDates = filteredGroupedItems.keys.toList()
      ..sort((a, b) => b.compareTo(a));
    final availableSubjects = _getAvailableSubjects(vm.items);
        ...
        AssignmentsSidebar(
          ...
          onSelectSubject: (subject) {
            setState(() {
              ...
              _selectedDate =
                  null; // сменили предмет, фильтр по дате сбрасываем
            });
          },
        ),
  ...
  Widget _buildDesktopContent(
    ...
    // если выбрана дата, фильтруем и по ней
            ...
            child: displayDates.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemCount: displayDates.fold<int>(
                      0,
                      (sum, date) =>
                          sum + 1 + (groupedItems[date]?.length ?? 0),
                    ),
                    itemBuilder: (context, index) {
                      int currentIndex = 0;
...
class _HomeworkCard extends StatelessWidget {
  ...
  Color _getSubjectColor(String subject, ColorScheme colorScheme) {
    ...
    final colors = [
      ...
      const Color(0xFFF59E0B), // янтарный
      const Color(0xFF10B981), // изумрудный
      const Color(0xFF06B6D4), // бирюзовый
  ...
  Widget build(BuildContext context) {
    ...
    final attachmentCount = item.files.isNotEmpty
        ? item.files.length
        : (item.attachCount ?? 0);
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      const SizedBox(width: 10),
                      Text(
                        _getFilesText(attachmentCount, l10n),
                        style: appFont(context,
                          fontSize: 13,
                          fontWeight: FontWeight.w500,
                          color: colorScheme.primary,
...
// заголовок с датой для десктопа
...
// карточка домашнего задания для десктопа, шире и подробнее
  ...
  Widget build(BuildContext context) {
    ...
    return Material(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(16),
        side: BorderSide(
          color: colorScheme.onSurface.withValues(alpha: isDark ? 0.07 : 0.08),
        ),
      ),
      ...
      child: InkWell(
        onTap: () {
          HapticFeedback.selectionClick();
          // на десктопе подробности показываем диалогом
          showDialog(
            context: context,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Row(
                      children: [
                        // цвет относится к предмету, а не к контуру карточки
                        Container(
                          width: 8,
                          height: 8,
                          decoration: BoxDecoration(
