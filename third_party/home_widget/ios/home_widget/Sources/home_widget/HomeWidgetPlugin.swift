  ...
  FlutterSceneLifeCycleDelegate
{
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    // регистрация во время запуска позволяет собрать модуль и для расширений приложения
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    if call.method == "setAppGroupId" {
      ...
      if let myArgs = args as? [String: Any?] {
        // сначала пробуем имя ios, затем общее; NSNull обрабатываем явно, оператор для nil его не заменит
    ...
    {
      // на ios временем управляет timeline виджета, плагин ничего не планирует и возвращает false
      result(false)
                  ...
                  if !intentData.isEmpty {
                    ...
                    for (internalPropertyName, rawValue) in intentData {
                      ...
                      if let intentParameter = rawValue as? _AnyIntentParameter {
                        // для WidgetConfigurationIntent извлекаем значение IntentParameter
                        value = intentParameter.anyWrappedValue
                      } else {
                        // обычное значение используем без разворачивания
                        value = rawValue
                      }
                      switch value {
                      case is NSNull:
                      ...
                      // типы Codable пробуем представить словарём
