...
/// боковая панель заданий для десктопа: календарь и превью по датам
class AssignmentsSidebar extends StatefulWidget {
  ...
  @override
  State<AssignmentsSidebar> createState() => _AssignmentsSidebarState();
}

class _AssignmentsSidebarState extends State<AssignmentsSidebar> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // список дат
          Expanded(
            child: widget.sortedDates.isEmpty
                ? Center(
                    child: Text(
                      l10n.noAssignments,
                      style: appFont(context,
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final items = widget.groupedItems[date] ?? [];
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${DateFormat('d MMM', 'ru').format(widget.startDate)} - ${DateFormat('d MMM', 'ru').format(widget.endDate)}',
                      style: appFont(context,
                        fontSize: 14,
                        fontWeight: FontWeight.w600,
                        color: colorScheme.onSurface,
  ...
  Widget _buildSubjectFilter(
      BuildContext context, ColorScheme colorScheme, bool isDark, AppLocalizations l10n) {
    if (widget.availableSubjects.isEmpty) return const SizedBox.shrink();
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Text(
                '${widget.totalCount} ${l10n.tasksLabel}',
                style: appFont(context,
                  fontSize: 13,
                  fontWeight: FontWeight.w600,
                  color: colorScheme.onSurface,
        ...
        SizedBox(
          ...
          child: Listener(
            onPointerSignal: (event) {
              if (event is PointerScrollEvent && _subjectScrollController.hasClients) {
                final maxExtent = _subjectScrollController.position.maxScrollExtent;
                if (maxExtent > 0) {
                  final newOffset = (_subjectScrollController.offset + event.scrollDelta.dy).clamp(0.0, maxExtent);
            ...
            child: ListView.builder(
              ...
              itemCount: widget.availableSubjects.length + 1, // плюс чип «все»
              itemBuilder: (context, index) {
              if (index == 0) {
                // чип «все»
                final isSelected = widget.selectedSubject == null;
                return Padding(
                  padding: const EdgeInsets.only(right: 8),
                  child: GestureDetector(
                    onTap: () {
                      HapticFeedback.selectionClick();
                      widget.onSelectSubject(null);
                    },
                    child: Container(
              ...
              final subject = widget.availableSubjects[index - 1];
...
/// строка с датой в боковой панели
