...
import '../widgets/marks_analytics_view.dart';
import '../widgets/marks_sidebar.dart';
import '../utils/teacher_chat.dart';
import '../providers/settings_provider.dart';
import '../utils/app_font.dart';
import '../utils/mark_color.dart';
import '../widgets/period_picker_list.dart';

enum MarksDisplayMode { list, table, analytics }
...
class _MarksViewState extends State<_MarksView>
  ...
  MarksDisplayMode _displayMode = MarksDisplayMode.list;
  ...
  Widget build(BuildContext context) {
    ...
    // на десктопе, если ничего не выбрано, открываем первый предмет
  ...
  Widget _buildMobileLayout(
    ...
    return Column(
      ...
      children: [
        ...
        // кнопка обновления
        if (_displayMode != MarksDisplayMode.analytics && vm.selectedPeriod != null && !vm.isLoading)
  ...
  /// десктопная вёрстка в две колонки
  Widget _buildDesktopLayout(
    ...
    if (_displayMode == MarksDisplayMode.table) {
      return Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _buildHeader(vm, colorScheme, l10n),
          ...
          if (vm.selectedPeriod != null && !vm.isLoading)
            _buildRefreshButton(vm, colorScheme, isDark, l10n),
          Expanded(
            child: vm.subjects.isNotEmpty
                ? _MarksTableView(subjects: vm.subjects, vm: vm, l10n: l10n)
    ...
    if (_displayMode == MarksDisplayMode.analytics) {
      final selectedUnitId = int.tryParse(_selectedSubjectId ?? '');
      final analyticsRefreshing = vm.isRefreshing ||
          (selectedUnitId != null && vm.isPlanSuccessLoading(selectedUnitId)) ||
          vm.analyticsUnitsLoading;
      ...
      return Row(
        children: [
          MarksSidebar(
            ...
            onRefresh: () {
              // в режиме аналитики кнопка обновления должна перетянуть график
              if (selectedUnitId != null) {
                vm.loadPlanSuccessForUnit(selectedUnitId, forceRefresh: true);
              } else {
                vm.loadAnalyticsUnits(forceRefresh: true);
              }
              // оценки тоже обновляем, список периодов приходит вместе с ними
              vm.refreshMarksData();
            },
            trailing: _buildViewToggle(colorScheme),
          ),
          Expanded(
            child: MarksAnalyticsView(
              controlledUnitId: _selectedSubjectId,
    ...
    return Row(
      children: [
        ...
        MarksSidebar(
          ...
          onSelectPeriod: (period) {
            vm.selectPeriod(period);
            // сменили период, выбор сбрасываем
            setState(() {
              _selectedSubjectId = null;
            });
          },
      ...
      child: Row(
        ...
        children: [
          _buildViewToggleButton(
            ...
            isSelected: _displayMode == MarksDisplayMode.list,
            colorScheme: colorScheme,
            onTap: () => setState(() => _displayMode = MarksDisplayMode.list),
          ),
          _buildViewToggleButton(
            icon: Icons.grid_view_rounded,
            isSelected: _displayMode == MarksDisplayMode.table,
            colorScheme: colorScheme,
            onTap: () => setState(() => _displayMode = MarksDisplayMode.table),
          ),
          _buildViewToggleButton(
            icon: Icons.insights_rounded,
            isSelected: _displayMode == MarksDisplayMode.analytics,
            colorScheme: colorScheme,
            onTap: () => setState(() => _displayMode = MarksDisplayMode.analytics),
          ),
        ],
      ),
    );
  ...
  Widget _buildContent(
          ...
          child: Column(
            ...
            children: [
              ...
              FilledButton(
                ...
                child: Text(l10n.retry, style: appFont(context, fontWeight: FontWeight.w500)),
              ),
            ],
          ),
        ),
    ...
    if (_displayMode == MarksDisplayMode.table) {
      return _MarksTableView(subjects: vm.subjects, vm: vm, l10n: l10n);
    }

    if (_displayMode == MarksDisplayMode.analytics) {
      return MarksAnalyticsView(initialUnitId: _selectedSubjectId);
    }
...
/// подробности по предмету на десктопе
            ...
            child: Column(
              ...
              children: [
                ...
                if (subject.teacher != null) ...[
                  ...
                  Builder(builder: (ctx) {
                    final chatEnabled = ctx
                        .watch<SettingsProvider>()
                        .teacherChatEnabled;
                    return GestureDetector(
                      behavior: HitTestBehavior.opaque,
                      onTap: chatEnabled
                          ? () => openChatWithTeacher(ctx, subject.teacher!)
                          : null,
                      child: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
...
/// карточка со статистикой для десктопа
...
/// строка с датой и оценками для десктопа
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              title: Text(l10n.excludeFromCalc, style: appFont(ctx)),
              subtitle: Text(l10n.markNotCounted, style: appFont(ctx)),
              onTap: () {
                Navigator.pop(ctx);
                vm.deleteOriginalMark(markId);
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(l10n.markExcludedMessage(mark.value), style: appFont(ctx)),
...
class _MarksTableView extends StatefulWidget {
  final List<SubjectData> subjects;
  final MarksViewModel vm;
  final AppLocalizations l10n;

  const _MarksTableView({required this.subjects, required this.vm, required this.l10n});

  @override
  State<_MarksTableView> createState() => _MarksTableViewState();
}

class _MarksTableViewState extends State<_MarksTableView>
    with TickerProviderStateMixin {
  final ValueNotifier<double> _hOffset = ValueNotifier(0.0);
  ...
  void _stopFling() {
    final c = _flingController;
    _flingController = null;
    c?.stop();
    c?.dispose();
  }

  void _onDragDown(DragDownDetails _, double __) {
    _stopFling();
  }

  void _onDragUpdate(DragUpdateDetails details, double maxOffset) {
    _hOffset.value =
        (_hOffset.value - details.delta.dx).clamp(0.0, maxOffset);
  }

  void _onDragEnd(DragEndDetails details, double maxOffset) {
    final velocity = -(details.primaryVelocity ?? 0);
    if (velocity.abs() < 200 || maxOffset <= 0) return;

    _stopFling();
    final controller = AnimationController.unbounded(
      vsync: this,
      value: _hOffset.value,
    );
    ...
    controller.addListener(() {
      ...
      final v = controller.value.clamp(0.0, maxOffset);
      _hOffset.value = v;
      if (v <= 0 || v >= maxOffset) controller.stop();
    });
    controller.animateWith(
      ClampingScrollSimulation(position: _hOffset.value, velocity: velocity),
    );
  }

  Color _getMarkColor(String mark) => colorForMark(mark);

  void _showSubjectDetail(BuildContext context, SubjectData subject) {
    if (isDesktop(context)) {
      ...
      showModalBottomSheet(
        ...
        builder: (ctx) => ChangeNotifierProvider.value(
          value: widget.vm,
          child: _SubjectDetailSheet(subject: subject, l10n: widget.l10n),
        ),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final List<Map<DateTime, List<MarkData>>> subjectMarksGrouped = [];

    for (var subject in widget.subjects) {
      final Map<DateTime, List<MarkData>> grouped = {};
      for (var m in subject.marks) {
        final markId =
            '${subject.id}_${m.date.millisecondsSinceEpoch}_${m.value}';
        if (widget.vm.isMarkDeleted(markId)) continue;
        final day = DateTime(m.date.year, m.date.month, m.date.day);
        allDatesSet.add(day);
        if (!grouped.containsKey(day)) grouped[day] = [];
        grouped[day]!.add(m);
      }
      subjectMarksGrouped.add(grouped);
    }

    final allDates = allDatesSet.toList()..sort();
    if (allDates.isEmpty) return const SizedBox.shrink();

    const double subjectColWidth = 140;
    const double dateColWidth = 48;
    const double rowHeight = 44;
    const double headerHeight = 44;
    final double totalDatesWidth = allDates.length * dateColWidth;
    ...
    Widget buildHScrollRow({required Widget child}) {
      return Expanded(
        child: ClipRect(
          child: ValueListenableBuilder<double>(
            valueListenable: _hOffset,
            builder: (context, offset, c) => Stack(
              children: [
                Positioned(
                  left: -offset,
                  top: 0,
                  bottom: 0,
                  width: totalDatesWidth,
                  child: c!,
                ),
              ],
            ),
            child: SizedBox(width: totalDatesWidth, child: child),
          ),
        ),
      );
    }
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final availableWidth = constraints.maxWidth - subjectColWidth;
        final maxOffset =
            (totalDatesWidth - availableWidth).clamp(0.0, double.infinity);

        return GestureDetector(
          onHorizontalDragDown: (d) => _onDragDown(d, maxOffset),
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final grouped = subjectMarksGrouped[index];

                    final hasChanges = widget.vm.hasChanges(subject.id);
                    final modifiedAvg =
                        widget.vm.calculateModifiedAverage(subject.id);
                    String baseAverage;
                    if (hasChanges && modifiedAvg != "-") {
                      baseAverage = modifiedAvg;
                    } else if (subject.average != "-") {
                      baseAverage = subject.average;
                    } else {
                      baseAverage = subject.calculatedAverage;
                    }
                    final avgColor =
                        grading.getAverageColorFromString(baseAverage);
                    ...
                    return Container(
                      ...
                      child: Row(
                        children: [
                          // колонка предметов, она не скроллится
                          GestureDetector(
                            onTap: () {
                              HapticFeedback.lightImpact();
                                ...
                                children: [
                                  ...
                                  if (baseAverage != "-") ...[
                                    ...
                                    Container(
                                      ...
                                      child: Text(
                                        baseAverage.length > 4
                                            ? baseAverage.substring(0, 4)
                                            : baseAverage,
                                        style: appFont(context,
                                          fontSize: 10,
                                          fontWeight: FontWeight.w700,
                          ...
                          // оценки, горизонтальный сдвиг общий с шапкой
                          buildHScrollRow(
                            child: Row(
                              children: allDates.map((date) {
                                final marks = grouped[date];
                                return Container(
                                  ...
                                  height: rowHeight,
                                  alignment: Alignment.center,
                                  child: marks != null && marks.isNotEmpty
                                      ? Column(
                                          mainAxisAlignment:
                                                ...
                                                if (isDesktop(context)) {
                                                  showDialog(
                                                    ...
                                                    builder: (ctx) => Dialog(
                                                      ...
                                                      child: ConstrainedBox(
                                                        constraints: const BoxConstraints(maxWidth: 500),
                                                        child: LessonDetailSheet(lesson: m.lesson, lessonDate: m.date),
                                                      ),
                                                    ),
                                                  );
                                                } else {
                                                  showModalBottomSheet(
                                                    ...
                                                    isScrollControlled: true,
                                                    backgroundColor: Colors.transparent,
                                                    builder: (ctx) => LessonDetailSheet(lesson: m.lesson, lessonDate: m.date),
                                                  );
                                                }
                                              },
                                              onLongPress: () {
                                                HapticFeedback.heavyImpact();
                                                _showMarkOptions(context, subject.id, m, markId);
                                              },
                                              child: Container(
                                                width: 28,
                                                height: marks.length > 1
                                                    ? 18
                                                    : 28,
                                                margin: marks.length > 1
                                                    ? const EdgeInsets
                                                        .symmetric(
                                                        vertical: 1)
                                                    : EdgeInsets.zero,
...
class _SubjectCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // группируем настоящие оценки по датам, скрытые не берём
    final Map<DateTime, List<MarkData>> grouped = {};
    for (var m in subject.marks) {
      final markId = '${subject.id}_${m.date.millisecondsSinceEpoch}_${m.value}';
      if (vm.isMarkDeleted(markId)) continue;
  ...
  Widget _buildAverageBadges(
    ...
    return Wrap(
      ...
      children: [
        ...
        // посчитанный нами средний, показываем если он разошёлся с api
        if (!hasChanges &&
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 20),
                Text(
                  '${l10n.markCaps} ${mark.value}', // регистр не идеальный, но читается нормально
                  style: appFont(ctx,
                    fontSize: 18,
                    fontWeight: FontWeight.w600,
                    color: colorScheme.onSurface,
...
class _AddEditMarkDialogState extends State<_AddEditMarkDialog>
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: Stack(
        children: [
        // размытие фона, когда диалог развёрнут
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Center(
                        child: Text(
                          'Выбрано: $_selectedMark', // строку стоит увести в локализацию
                          style: appFont(context,
                            fontSize: 14,
                            fontWeight: FontWeight.w600,
                  ...
                  children: [
                    ...
                    Builder(builder: (ctx) {
                      ...
                      return _DetailCard(
                        ...
                        colorScheme: colorScheme,
                        isDark: isDark,
                        onTap: chatEnabled && subject.teacher != null
                            ? () => openChatWithTeacher(
                                ctx, subject.teacher!)
                            : null,
                      );
